...
(async () => {
  ...
  await page.getByRole('link', { name: 'ย้อนกลับ' }).waitFor();
  ...
  await page.getByRole('heading', { name: 'ชวนทุกคนมีส่วนร่วมในทุกช่วงการเรียนรู้' }).waitFor();
  ...
  await assertFont('en', '-apple-system');
  await page.goto(`${webUrl}/login`);
