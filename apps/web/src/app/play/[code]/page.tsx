...
function mergeWordCloudUpdate(current: State | undefined, next: State) {
  ...
  if (
    !current?.question ||
    !merged.question ||
    current.question.id !== merged.question.id
  )
    return merged;
  const voted = new Map(
    current.question.entries.map((entry) => [entry.id, entry.voted]),
  );
  return {
    ...merged,
    question: {
...
function mergePrivateState(current: State | undefined, next: State) {
  ...
  if (!sameQuestion && !completesCurrentQuestion) return next;
  return {
    ...next,
    ...
    <main className="page-shell">
      <div className="page-content max-w-3xl">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <p role="status" aria-live="polite" className="badge text-xs">
            {t(`common.${connection}`)}
          </p>
        </div>
        ...
        {bootstrapFailed && !state && (
          <section className="panel mt-8 text-center">
            <p role="alert" className="alert-error">
              {t(`errors.${errorCode || "REQUEST_FAILED"}`)}
            </p>
            <button
              type="button"
        ...
        state.activityType === "WORD_CLOUD" ? (
          <section className="panel mt-6">
            ...
            <WordCloudResults
              entries={state.question.entries}
              totalVotes={state.question.totalVotes}
          ...
          <section className="panel mt-6">
            ...
            {pollResult ? (
              <>
                <p className="mt-4 text-center text-xl font-semibold text-slate-700">
                  {pollResult.text}
                </p>
                    ...
                    >
                      ...
                      <div className="mt-2 h-3 overflow-hidden rounded-full bg-sky-100">
                        <div
                          ...
                          style={{
                            width: `${pollResult.responseCount ? (choice.count / pollResult.responseCount) * 100 : 0}%`,
                          }}
                        />
                      </div>
                    </div>
          ...
          state.phase !== "COMPLETED" && (
            <section className="panel mt-8 sm:p-10">
              ...
              <h1 className="mt-3 break-words text-3xl font-semibold leading-tight tracking-tight text-[#1d1d1f] sm:text-5xl">
                {state.question.text}
              </h1>
              {(state.phase === "ACTIVE" || state.phase === "REVEALED") &&
                state.activityType !== "WORD_CLOUD" && (
                  <div className="mt-7 grid gap-3">
                    {state.question.choices.map((choice) => {
                      ...
                      const correct =
                        ...
                        choice.id === state.correctChoiceId;
                      const wrong =
                        state.phase === "REVEALED" && selected && !correct;
                      return (
                        <button
                          type="button"
                    ...
                    {!state.wordSubmitted && (
                      <>
                        <div className="flex flex-col gap-3 sm:flex-row">
                          <label
                            htmlFor="word-cloud-response"
                            className="sr-only"
                          >
                            {t("wordCloud.addResponse")}
                          </label>
                          <input
                            id="word-cloud-response"
                            value={word}
                            maxLength={30}
                            disabled={submitting || !connected}
                            onChange={(event) => setWord(event.target.value)}
                            className="form-input mt-0 min-w-0 flex-1"
                            placeholder={t("wordCloud.addResponse")}
                            aria-describedby="word-cloud-response-hint"
                          />
                          <button
                            type="button"
                            disabled={!word.trim() || submitting || !connected}
                            onClick={() =>
                              wordAction(RoomEvents.wordCloudSubmit, {
                                text: word.trim(),
                              })
                            }
                        ...
                        <p
                          id="word-cloud-response-hint"
                          className="mt-2 flex justify-between gap-3 text-sm text-slate-600"
                        >
                          <span>{t("wordCloud.responseHint")}</span>
                      ...
                      {entries.map((entry) =>
                        entry.isOwn ? (
                          ...
                          <button
                            ...
                            disabled={submitting || !connected}
                            onClick={() =>
                              wordAction(RoomEvents.wordCloudVote, {
                                entryId: entry.id,
                              })
                            }
              ...
              {state.activityType === "QUIZ" && state.phase === "REVEALED" && (
                ...
                >
                  {!state.selectedChoiceId
                    ? t("player.noAnswer")
                    : state.selectedChoiceId === state.correctChoiceId
                      ? t("player.correctFeedback")
                      : t("player.incorrectFeedback")}
                </p>
              )}
            </section>
          )
        ...
        {errorCode && (!bootstrapFailed || state) && (
